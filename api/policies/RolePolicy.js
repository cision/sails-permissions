/*
Overrides sails-permissions RolePolicy.js
*/

/*TODO: Move this into an updated NPM module and then include as a dependancy.*/
/**
 * RolePolicy
...
module.exports = function (req, res, next) {
  ...
  if (!_lodash2['default'].contains(['update', 'delete'], action) && req.options.modelDefinition.attributes.owner) {
    ...
    //TODO: Determine why this block is erroring and what it is used for.
  ...
  PermissionService.findTargetObjects(req).then(function (objects) {
    ...
    return PermissionService.isAllowedToPerformAction(objectData, req.user, action, ModelService.getTargetModelName(req), req.body).then(function (hasUserPermissions) {
      if (hasUserPermissions) {
        return next();
      }
      if (PermissionService.hasForeignObjects(objectData, req.user)) {
        return res.send(403, {
